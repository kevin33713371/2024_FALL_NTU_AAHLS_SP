...
tr.send<4>((char*)__xlx_apatb_param_c, 1024, 0);
}
#else
try {
...
if (__xlx_apatb_param_a) {
  tr.import<4>((char*)__xlx_apatb_param_a, 1024, 0);
}
...
if (__xlx_apatb_param_b) {
  tr.import<4>((char*)__xlx_apatb_param_b, 1024, 0);
}
...
if (__xlx_apatb_param_c) {
  tr.import<4>((char*)__xlx_apatb_param_c, 1024, 0);
}
aesl_fh.write(AUTOTB_TVIN_gmem2, tr.p, tr.tbytes);
tcl_file.set_num(1024, &tcl_file.gmem2_depth);
}
#else
